...
public:
    ...
    VectorStat actual_writes_counter_vec;
    VectorStat swaps_counter_vec;
    ScalarStat actual_writes_counter;
    ScalarStat erased_writes_counter;
    ScalarStat write_from_write;
    ScalarStat first_write_requests;

    typedef int CounterBank_t;
    typedef CounterBank_t* CountersRank_t;
    typedef CountersRank_t* ActualWriteCountersTree_t;
    ActualWriteCountersTree_t actual_writes_counter_tree = nullptr;
    ...
    {
        swap_enable = configs["swap"] == "on" ? true : false;
        std::cout << "swap_enable: " << swap_enable << std::endl;
        if (swap_enable) {
            X_value = std::stol(configs["X_value"]);
            std::cout << "X_value: " << X_value << std::endl;

            /* Initializing the actual write counter tree */
            actual_writes_counter_tree;
            int rank_count = channel->spec->org_entry.count[int(T::Level::Rank)];
            int bank_count = channel->spec->org_entry.count[int(T::Level::Bank)];

            actual_writes_counter_tree = new CountersRank_t[rank_count];
            for (int rank_id = 0; rank_id < rank_count; rank_id++) {
                actual_writes_counter_tree[rank_id] = new CounterBank_t[bank_count];
            }
        }
        ...
        actual_writes_counter
            .name("actual_writes_"+to_string(channel->id))
            .desc("actual_writes per channel.")
            .precision(0)
            ;

        swaps_counter_vec
            .init(configs.get_core_num())
            .name("swaps_counter_vec_" + to_string(channel->id))
            .desc("swaps_counter_vec per channel.")
            .precision(0)
            ;
        actual_writes_counter_vec
            .init(configs.get_core_num())
            .name("actual_writes_counter_vec_"+to_string(channel->id))
            .desc("actual_writes_counter_vec per channel.")
            .precision(0)
            ;

        erased_writes_counter
            .name("erased_writes_counter_"+to_string(channel->id))
            .desc("erased_writes_counter per channel.")
            .precision(0)
            ;
        write_from_write
            .name("write_from_write_"+to_string(channel->id))
            .desc("write_from_write per channel.")
            .precision(0)
            ;
        first_write_requests
            .name("first_write_requests_"+to_string(channel->id))
            .desc("first_write_requests per channel.")
            .precision(0)
            ;
        ...
        if (req->is_first_command) {
            ...
            if (req->type == Request::Type::READ) {
              ...
              if (is_row_hit(req)) {
                  ...
                  ++row_hits;
                  req->rowRelation = Request::RowRelation::HIT;
//                  std::cout << "Write hit" << std::endl;
              } else if (is_row_open(req)) {
                  ++write_row_conflicts[coreid];
                  ++row_conflicts;
                  req->rowRelation = Request::RowRelation::CONFLICT;
//                  std::cout << "Write conflict" << std::endl;
              } else {
                  ++write_row_misses[coreid];
                  ++row_misses;
                  req->rowRelation = Request::RowRelation::MISS;
//                  std::cout << "Write Miss" << std::endl;
        ...
        issue_cmd(cmd, get_addr_vec(cmd, req), req->coreid, req->rowRelation);

        if (req->type == Request::Type::WRITE and cmd == T::Command::WR)
            ++write_from_write;
...
private:
    ...
    void cmd_issue_autoprecharge(typename T::Command& cmd,
        ...
        if(channel->spec->is_accessing(cmd) && int(cmd) == int(PCM::Command::WR)) {
    ...
    void issue_cmd(typename T::Command cmd, const vector<int>& addr_vec, int coreid = -1, Request::RowRelation rowRelation = Request::RowRelation::UNSET)
    {
        /* Working of WLAD:
         * If the command being issued is a write command, the controller will increment it's actual_write_counter (actual
         * in the sense that these requests are actually issued to the controller rather than being queued to wqueue).
         * Next, if the the actual_write_counter value is a multiple of the X_value the controller will insert a swap
         * request to the swap_queue.
         * Now, whenever the controller will tick (in Controller::Tick()) next the swap command will be issued
         * (honouring appropiate timings mentioned in src/PCM.h and src/PCM.cpp */
        if (int(cmd) == int(T::Command::WR) and swap_enable) {
//            std::cout << "Write command" << std::endl;
            assert(rowRelation != Request::RowRelation::UNSET or rowRelation != Request::RowRelation::MAX);
            assert(coreid != -1);

            if (rowRelation == Request::RowRelation::CONFLICT or rowRelation == Request::RowRelation::MISS) {
                ++actual_writes_counter;
                ++actual_writes_counter_vec[coreid];
                assert(actual_writes_counter_tree != nullptr);

                int rank = addr_vec[int(T::Level::Rank)];
                int bank = addr_vec[int(T::Level::Bank)];
                ++actual_writes_counter_tree[rank][bank];

//                std::cout << "Actual write" << std::endl;
                if ((actual_writes_counter_tree[rank][bank])%X_value == 0) {
                    ++swaps_counter_vec[coreid];
                    swap_queue.push(addr_vec); // Add the address vector
//                    std::cout << "Inserted swap request for " << addr_vec[0] << " " << addr_vec[1] << " " << addr_vec[2]
//                              << " new queue size = " << swap_queue.size()
//                              << " actual_writes_counter = " << actual_writes_counter.value() << std::endl;
                }
            }
        }
