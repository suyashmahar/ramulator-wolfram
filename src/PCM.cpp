...
void PCM::init_prereq()
{
    ...
    // REF
    prereq[int(Level::Bank)][int(Command::SWAP)] = [] (DRAM<PCM>* node, Command cmd, int id) {
...
void PCM::init_lambda()
{
    ...
    lambda[int(Level::Rank)][int(Command::SWAP)] = [] (DRAM<PCM>* node, int id) {};
...
//    t[int(Command::SRX)].push_back({Command::REF, 1, s.nXS});
    ...
    t[int(Command::SWAP)].push_back({Command::PRE, 1, 60});
    t[int(Command::SWAP)].push_back({Command::WR, 1, 60});
    t[int(Command::SWAP)].push_back({Command::RD, 1, 60});

    t[int(Command::WR)].push_back({Command::SWAP, 1, 25});
